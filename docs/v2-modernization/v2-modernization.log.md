...
## `docs-and-migration` — 2026-05-21 19:37:16

**Status:** needs-review
**Summary:** Rewrote the README for the shipped v2 API, added a top-level `MIGRATION.md` covering every v1-to-v2 transition, and replaced the old Chakra-based demo app with a simpler Vite workspace app that exercises the current `useFavicon`, `useIsDark`, and `useIsAway` exports. Added `__tests__/documentationExamples.test.tsx` so the README examples and migration escape hatches are covered by the library test suite.
**Deviations:** The site workspace still carries its legacy Chakra dependencies in `site/package.json`, but the demo itself no longer depends on them. I left dependency pruning out of this slice because the user-facing docs/site contract is already green and this slice has a human checkpoint.
**Handoff:** Human review should focus on clarity and completeness of [README.md](/Users/arielbk/Documents/side_projects/use-favicon/README.md), [MIGRATION.md](/Users/arielbk/Documents/side_projects/use-favicon/MIGRATION.md), and the rebuilt demo UI in `site/`. The automated feedback loop passed at repo scope: `pnpm run typecheck`, `pnpm run test`, and `pnpm run build`.
